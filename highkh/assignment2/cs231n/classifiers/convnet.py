...
class ConvNet(object):
  ...
  def add_spatial_dropout(self, dropout_param = { 'p': 0.5 }):
    def forward(X, mode):
      param = dropout_param
      param['mode'] = mode
      return spatial_dropout_forward(X, param)

    self.forwards.append(forward)
    self.backwards.insert(0, lambda dout, cache: (spatial_dropout_backward(dout, cache), {}))
